...
go_folder = paste0(here::here(), "/SBM/snakemake/cache/GO/fdr-1e-3")
go_plots = paste0(here::here(), "/SBM/snakemake/cache/GO_plots/fdr-1e-3/")
...
llply(tissues, function(tissue){
    ...
    for(block in en_s$Name){
        ...
        save_plot(file.path(plot_path, paste0("Level_", level, "_", block, ".png")), p1, base_height = 9)
    }
}, .parallel = TRUE)
